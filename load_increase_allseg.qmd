...
hymodat <- mohydat |> 
  filter(year >= 2000 & year <= 2021) |>
  filter(bay_segment %in% segs) |> 
  select(year, month, bay_segment, hy_load = hy_load_106_m3_mo) |> 
  mutate(bay_segment = factor(bay_segment, levels = segs))
hydat <- hymodat |> 
  summarise(
    hy_load = sum(hy_load),
...
tnmod <- lm(tn_load ~ year*bay_segment, data = npslddat)

# get predictions for years 1985:2040
preds <- data.frame(year = 2000:2040) |> 
  crossing(bay_segment = factor(segs, levels = segs))
preds <- preds |> 
  mutate(
...
p <- ggplot(npslddat, aes(x = year, y = tn_load)) +
  ...
  labs(title = 'Total Nitrogen NPS Loads by bay segment',
       x = 'Year',
       y = 'Load (tons / yr)'
  ) +
  theme_minimal()
...
npsper <- tnanndat |> 
  ...
  select(year, bay_segment, nps_percent)
npsper
```

A daily interpolated load file for the historical monthly data is 

The NPS percent each year is joined to a daily interpolated load file and the proportion of the TN load that is NPS is extracted and changed by the estimated percent change from above. 

```{r}
npschg <- means |> 
  select(bay_segment, percent_change) 

dlydat <- crossing(
    date = seq.Date(as.Date('2000-01-01'), as.Date('2021-12-31'), by = 'day'), 
    bay_segment = factor(segs, levels = segs), 
    source = unique(mosdat$source)
  ) |> 
  mutate(
    month = month(date), 
    year = year(date), 
    ndays = days_in_month(date)
  ) |> 
  left_join(mosdat, by = c('bay_segment', 'source', 'month', 'year')) |> 
  left_join(npschg, by = c('bay_segment')) |> 
  select(date, bay_segment, source, month, year, ndays, tn_load, percent_change) |> 
  mutate(
    tn_load = tn_load / ndays,
    tn_loadchg = case_when(
      source == 'NPS' ~ tn_load + (tn_load * percent_change / 100), 
      T ~ tn_load
    )
  )
```
...
```{r}
toplo <- dlydat |> 
  select(date, bay_segment, original = tn_load, change = tn_loadchg) |> 
