...
chldat1 <- epcwq3[ which(epcwq3$param=="Chla"  & epcwq3$subseg=="CE"),
                   c('date','value') ]
...
ssa_chl_otb_ce <- list( dat = data.frame( month = chldat$month,
                                        ts = chldat$value,
...
save( ssa_chl_otb_ce, file = "../data/ssa_chl_otb_ce.RData" )
