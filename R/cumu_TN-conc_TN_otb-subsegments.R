...
png( "../figs/cumu_TN-conc_TN_otb-subsegments.png",
...
# TN data (EPC)
epcwq3.sub <- epcwq3[ which( epcwq3$param=="TN"
                             & epcwq3$subseg==subseg
                       ), ]
epcwq3.sub$month <- floor_date( epcwq3.sub$date, unit = 'month' ) 
tndat <- epcwq3.sub |> group_by(month) |> dplyr::summarise( TNconc = mean(value) ) |> as.data.frame()
wqdat <- inner_join( tndat, loaddat, by = 'month' )

# Define function to summarize TN distribution
distn <- function( x, max_TN ){
  # subset TN data associated with TN values at or below max_TN
  this <- x$TNconc[ which( x$TN <= max_TN ) ]
...
# Calculate TN distn statistics
# initiate dataframe
TN_TN <- data.frame( TN = seq(10,200,10),
                      mean = NA,
                      min = NA,
                      lwr = NA,
                      upr = NA,
...
# populate rows
for( i in 1:nrow(TN_TN) ){
  TN_TN[i,2:6] <- distn( wqdat, max_TN = TN_TN[i,1] )
}  # // end i loop

# Plot TN distn as a function of TN_max
plot( mean ~ TN, data = TN_TN, las = 1,
      type = 'l', lwd = 2, col = rgb(0,0.2,0.6,0.7),
      main = paste0( 
        # subseg,
        " OTB\nMonthly max TN conc distn ~ TN load" ),
      ylim = c(0,2.5), yaxt = 'n',
      ylab = "TN concentration (mg/L)", xlab = "", xlim = c(10,200)
)
...
abline( v = TN_TN$TN, col = rgb(0,0,0,0.1) )
polygon( x = c( TN_TN$TN, rev(TN_TN$TN) ),
         y = c( TN_TN$min, rev(TN_TN$max) ),
         col = rgb(0,0.2,0.6,0.1), border = rgb(0,0,0,0) )
polygon( x = c( TN_TN$TN, rev(TN_TN$TN) ),
         y = c( TN_TN$lwr, rev(TN_TN$upr) ),
         col = rgb(0,0.2,0.6,0.2), border = rgb(0,0,0,0) )
segments( x0 = 40,
          y0 = 0, y1 = TN_TN$upr[which(TN_TN$TN==40)],
          lty = 2, col = 2 )
text( x = 40, y = TN_TN$upr[which(TN_TN$TN==40)],
      col = 2, labels = "40 tons", pos = 4, srt = 90 )
segments( x0 = 0, x1 = 40,
          y0 = TN_TN$mean[which(TN_TN$TN==40)],
          lty = 2, col = 2 )
segments( x0 = 0, x1 = 40,
          y0 = TN_TN$lwr[which(TN_TN$TN==40)],
          lty = 2, col = 2 )
segments( x0 = 0, x1 = 40,
          y0 = TN_TN$upr[which(TN_TN$TN==40)],
          lty = 2, col = 2 )
