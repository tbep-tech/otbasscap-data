# OTB TP load signal processing with SSA
#
rm(list=ls(all=TRUE)) 
...
loads <- loads[ which(loads$param=="TP load"), ]

# Plot time series
  plot( value ~ date, data = loads, las = 1,
        type = 'l', lwd = 2, col = rgb(0,0,0,0.6),
        main = "TN load ts", xlab = '', ylab = 'TN (tons)' )
  abline( h = axTicks(2), col = rgb(0,0,0,0.1) )
  abline( v = seq.Date( min(loads$date),
                        max(loads$date)+60, 'year' ),
          col = rgb(0,0,0,0.1) )
  points( value ~ date, data = loads, pch = 21, lwd = 2, cex = 0.5,
...
##
  ...
  obj |> plot( type = 'wcor' )
  wcor(obj,groups = 1:40) |> plot()


# SSA grouping
##
  # Specify signal component groups
  grp <- list( c(1,2), c(3,4), c(5,7), c(6,8), c(9,10) )
...
# Export ts, signal, noise
  ssa_TP_otb <- list( dat = data.frame( month = loads$date,
                                        ts = loads$value,
                                        signal = signal + mean(loads$value),
                                        noise = noise + mean(loads$value) ),
                   grp = grp,
                   sigstrength = sigstrength 
                   )
  save( ssa_TP_otb, file = "../data/ssa_TP_otb.RData" )
