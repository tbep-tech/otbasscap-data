...
load( "../data/Pyro.Rdata")
  ...
  pyro2 <- pyro[ which(pyro$yr>=2012) , c("date","pyro") ]
...
##
  ...
  grp <- list( c(1,2), c(3,4,8,10), c(5,6,7,9) )
...
# Export ts, signal, noise
  ssa_pyro_otb <- list( dat = data.frame( month = pyrodat$month,
                                     ts = pyrodat$logcells,
  ...
  save( ssa_pyro_otb, file = "../data/ssa_pyro_otb.RData" )
