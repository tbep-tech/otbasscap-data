...
chldat1 <- epcwq3[ which(epcwq3$param=="Chla"  & epcwq3$subseg=="SE"),
                   c('date','value') ]
...
grp <- list( c(1,2), c(3,4,5,6), c(7,8) )
...
ssa_chl_otb_se <- list( dat = data.frame( month = chldat$month,
                                        ts = chldat$value,
...
save( ssa_chl_otb_se, file = "../data/ssa_chl_otb_se.RData" )
