...
chldat1 <- epcwq3[ which(epcwq3$param=="Chla"  & epcwq3$subseg=="NE"),
                   c('date','value') ]
...
ssa_chl_otb_ne <- list( dat = data.frame( month = chldat$month,
                                        ts = chldat$value,
...
save( ssa_chl_otb_ne, file = "../data/ssa_chl_otb_ne.RData" )
