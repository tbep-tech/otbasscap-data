...
chldat1 <- epcwq3[ which(epcwq3$param=="Chla"  & epcwq3$subseg=="NW"),
                   c('date','value') ]
...
ssa_chl_otb_nw <- list( dat = data.frame( month = chldat$month,
                                        ts = chldat$value,
...
save( ssa_chl_otb_nw, file = "../data/ssa_chl_otb_nw.RData" )
