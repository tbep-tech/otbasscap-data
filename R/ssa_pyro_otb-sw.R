...
pyro2 <- pyro[ which(pyro$yr>=2012 & pyro$subsegment=="SW") ,
               c("date","pyro") ]
...
grp <- list( c(1,2), c(3,4), c(5,6,7,8) )
...
ssa_pyro_otb_sw <- list( dat = data.frame( month = pyrodat$month,
                                      ts = pyrodat$logcells,
...
save( ssa_pyro_otb_sw, file = "../data/ssa_pyro_otb_sw.RData" )