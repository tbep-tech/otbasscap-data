...
plot.CCM.results <- function( CCM.OUTPUT, CCM.RHO, DELAY.SEQ=delay.seq, PVAL, MAIN,
                              XLAB = FALSE, YLAB = FALSE, YMIN = 0.20, XSEQ = c(-7:0) ){
...
# OTB chl xmap salinity
ccm.otb.chl.sal <- delay.CCM( LIBRARY='chl_otb', TARGET='sal_otb',
                             EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb"),]) )
ccm.otb.chl.sal.rho <- plot.CCM( ccm.otb.chl.sal )
ccm.otb.chl.sal.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = 0 )
ccm.otb.chl.sal.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = -1 ) 
ccm.otb.chl.sal.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = -2 ) 
ccm.otb.chl.sal.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = -3 ) 
ccm.otb.chl.sal.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = -4 ) 
ccm.otb.chl.sal.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = -5 ) 
ccm.otb.chl.sal.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = -6 ) 
ccm.otb.chl.sal.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.sal, CCM.RHO=ccm.otb.chl.sal.rho, DELAY = -7 ) 

# NW chl xmap salinity
ccm.nw.chl.sal <- delay.CCM( LIBRARY='chl_otb_nw', TARGET='sal_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_nw"),]) )
ccm.nw.chl.sal.rho <- plot.CCM( ccm.nw.chl.sal )
ccm.nw.chl.sal.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = 0 )
ccm.nw.chl.sal.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = -1 ) 
ccm.nw.chl.sal.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = -2 ) 
ccm.nw.chl.sal.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = -3 ) 
ccm.nw.chl.sal.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = -4 ) 
ccm.nw.chl.sal.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = -5 ) 
ccm.nw.chl.sal.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = -6 ) 
ccm.nw.chl.sal.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.sal, CCM.RHO=ccm.nw.chl.sal.rho, DELAY = -7 ) 

# NE chl xmap salinity
ccm.ne.chl.sal <- delay.CCM( LIBRARY='chl_otb_ne', TARGET='sal_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_ne"),]) )
ccm.ne.chl.sal.rho <- plot.CCM( ccm.ne.chl.sal )
ccm.ne.chl.sal.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = 0 )
ccm.ne.chl.sal.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = -1 ) 
ccm.ne.chl.sal.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = -2 ) 
ccm.ne.chl.sal.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = -3 ) 
ccm.ne.chl.sal.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = -4 ) 
ccm.ne.chl.sal.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = -5 ) 
ccm.ne.chl.sal.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = -6 ) 
ccm.ne.chl.sal.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.sal, CCM.RHO=ccm.ne.chl.sal.rho, DELAY = -7 )

# CW chl xmap salinity
ccm.cw.chl.sal <- delay.CCM( LIBRARY='chl_otb_cw', TARGET='sal_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_cw"),]) )
ccm.cw.chl.sal.rho <- plot.CCM( ccm.cw.chl.sal )
ccm.cw.chl.sal.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = 0 )
ccm.cw.chl.sal.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = -1 ) 
ccm.cw.chl.sal.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = -2 ) 
ccm.cw.chl.sal.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = -3 ) 
ccm.cw.chl.sal.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = -4 ) 
ccm.cw.chl.sal.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = -5 ) 
ccm.cw.chl.sal.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = -6 ) 
ccm.cw.chl.sal.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.sal, CCM.RHO=ccm.cw.chl.sal.rho, DELAY = -7 ) 

# CE chl xmap salinity
ccm.ce.chl.sal <- delay.CCM( LIBRARY='chl_otb_ce', TARGET='sal_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_ce"),]) )
ccm.ce.chl.sal.rho <- plot.CCM( ccm.ce.chl.sal )
ccm.ce.chl.sal.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = 0 )
ccm.ce.chl.sal.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = -1 ) 
ccm.ce.chl.sal.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = -2 ) 
ccm.ce.chl.sal.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = -3 ) 
ccm.ce.chl.sal.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = -4 ) 
ccm.ce.chl.sal.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = -5 ) 
ccm.ce.chl.sal.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = -6 ) 
ccm.ce.chl.sal.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.sal, CCM.RHO=ccm.ce.chl.sal.rho, DELAY = -7 ) 

# SW chl xmap salinity
ccm.sw.chl.sal <- delay.CCM( LIBRARY='chl_otb_sw', TARGET='sal_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_sw"),]) )
ccm.sw.chl.sal.rho <- plot.CCM( ccm.sw.chl.sal )
ccm.sw.chl.sal.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = 0 )
ccm.sw.chl.sal.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = -1 ) 
ccm.sw.chl.sal.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = -2 ) 
ccm.sw.chl.sal.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = -3 ) 
ccm.sw.chl.sal.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = -4 ) 
ccm.sw.chl.sal.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = -5 ) 
ccm.sw.chl.sal.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = -6 ) 
ccm.sw.chl.sal.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.sal, CCM.RHO=ccm.sw.chl.sal.rho, DELAY = -7 ) 

# SE chl xmap salinity
ccm.se.chl.sal <- delay.CCM( LIBRARY='chl_otb_se', TARGET='sal_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_se"),]) )
ccm.se.chl.sal.rho <- plot.CCM( ccm.se.chl.sal )
ccm.se.chl.sal.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = 0 )
ccm.se.chl.sal.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = -1 ) 
ccm.se.chl.sal.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = -2 ) 
ccm.se.chl.sal.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = -3 ) 
ccm.se.chl.sal.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = -4 ) 
ccm.se.chl.sal.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = -5 ) 
ccm.se.chl.sal.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = -6 ) 
ccm.se.chl.sal.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.sal, CCM.RHO=ccm.se.chl.sal.rho, DELAY = -7 ) 
...
# Export outputs
save.image( file = "../data/ccm_chl00_sal.RData" )
write.csv( df.pval, file = "../data/ccm_chl00_sal_pvals.csv", row.names = FALSE )


# CCM figures --------------------------------------------------------------
png( "../figs/ccm_chl00_sal.png", width = 6, height = 10, units = 'in', res = 600 )
par(mfrow=c(4,2), mar=c(4,4.5,2,1))

plot.CCM.results( CCM.OUTPUT=ccm.otb.chl.sal,
                  CCM.RHO=ccm.otb.chl.sal.rho,
                  MAIN = "OTB segment", YLAB = TRUE,
                  PVAL=c(ccm.otb.chl.sal.pval.d0$pval, 
                         ccm.otb.chl.sal.pval.d1$pval, 
                         ccm.otb.chl.sal.pval.d2$pval, 
                         ccm.otb.chl.sal.pval.d3$pval, 
                         ccm.otb.chl.sal.pval.d4$pval, 
                         ccm.otb.chl.sal.pval.d5$pval, 
                         ccm.otb.chl.sal.pval.d6$pval, 
                         ccm.otb.chl.sal.pval.d7$pval ) )
plot(c(0,1),c(0,1),bty='n',xaxt='n',yaxt='n',xlab='',ylab='',col=rgb(0,0,0,0))
text( x = 0, y = 0.95, pos = 4, cex = 1.3,
      labels = "Chlorophyll-a xmap salinity" )
...
plot.CCM.results( CCM.OUTPUT=ccm.nw.chl.sal,
                  CCM.RHO=ccm.nw.chl.sal.rho,
                  MAIN = "NW sub-segment", YLAB = TRUE,
                  PVAL=c(ccm.nw.chl.sal.pval.d0$pval, 
                         ccm.nw.chl.sal.pval.d1$pval, 
                         ccm.nw.chl.sal.pval.d2$pval, 
                         ccm.nw.chl.sal.pval.d3$pval, 
                         ccm.nw.chl.sal.pval.d4$pval, 
                         ccm.nw.chl.sal.pval.d5$pval, 
                         ccm.nw.chl.sal.pval.d6$pval, 
                         ccm.nw.chl.sal.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.ne.chl.sal,
                  CCM.RHO=ccm.ne.chl.sal.rho,
                  MAIN = "NE sub-segment",
                  PVAL=c(ccm.ne.chl.sal.pval.d0$pval, 
                         ccm.ne.chl.sal.pval.d1$pval, 
                         ccm.ne.chl.sal.pval.d2$pval, 
                         ccm.ne.chl.sal.pval.d3$pval, 
                         ccm.ne.chl.sal.pval.d4$pval, 
                         ccm.ne.chl.sal.pval.d5$pval, 
                         ccm.ne.chl.sal.pval.d6$pval, 
                         ccm.ne.chl.sal.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.cw.chl.sal,
                  CCM.RHO=ccm.cw.chl.sal.rho,
                  MAIN = "CW sub-segment", YLAB = TRUE,
                  PVAL=c(ccm.cw.chl.sal.pval.d0$pval, 
                         ccm.cw.chl.sal.pval.d1$pval, 
                         ccm.cw.chl.sal.pval.d2$pval, 
                         ccm.cw.chl.sal.pval.d3$pval, 
                         ccm.cw.chl.sal.pval.d4$pval, 
                         ccm.cw.chl.sal.pval.d5$pval, 
                         ccm.cw.chl.sal.pval.d6$pval, 
                         ccm.cw.chl.sal.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.ce.chl.sal,
                  CCM.RHO=ccm.ce.chl.sal.rho,
                  MAIN = "CE sub-segment",
                  PVAL=c(ccm.ce.chl.sal.pval.d0$pval, 
                         ccm.ce.chl.sal.pval.d1$pval, 
                         ccm.ce.chl.sal.pval.d2$pval, 
                         ccm.ce.chl.sal.pval.d3$pval, 
                         ccm.ce.chl.sal.pval.d4$pval, 
                         ccm.ce.chl.sal.pval.d5$pval, 
                         ccm.ce.chl.sal.pval.d6$pval, 
                         ccm.ce.chl.sal.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.sw.chl.sal,
                  CCM.RHO=ccm.sw.chl.sal.rho,
                  MAIN = "SW sub-segment", YLAB = TRUE, XLAB = TRUE,
                  PVAL=c(ccm.sw.chl.sal.pval.d0$pval, 
                         ccm.sw.chl.sal.pval.d1$pval, 
                         ccm.sw.chl.sal.pval.d2$pval, 
                         ccm.sw.chl.sal.pval.d3$pval, 
                         ccm.sw.chl.sal.pval.d4$pval, 
                         ccm.sw.chl.sal.pval.d5$pval, 
                         ccm.sw.chl.sal.pval.d6$pval, 
                         ccm.sw.chl.sal.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.se.chl.sal,
                  CCM.RHO=ccm.se.chl.sal.rho,
                  MAIN = "SE sub-segment", XLAB = TRUE,
                  PVAL=c(ccm.se.chl.sal.pval.d0$pval, 
                         ccm.se.chl.sal.pval.d1$pval, 
                         ccm.se.chl.sal.pval.d2$pval, 
                         ccm.se.chl.sal.pval.d3$pval, 
                         ccm.se.chl.sal.pval.d4$pval, 
                         ccm.se.chl.sal.pval.d5$pval, 
                         ccm.se.chl.sal.pval.d6$pval, 
                         ccm.se.chl.sal.pval.d7$pval ) )
