# OTB TN load signal processing with SSA
#
rm(list=ls(all=TRUE)) 
...
##
  ...
  grp <- list( c(1,2), c(3,4), c(5,6), c(7,8), c(9,10) )
...
# Export ts, signal, noise
  ssa_TN_otb <- list( dat = data.frame( month = loads$date,
                                        ts = loads$value,
  ...
  save( ssa_TN_otb, file = "../data/ssa_TN_otb.RData" )
