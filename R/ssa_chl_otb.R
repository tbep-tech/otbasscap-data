...
chldat1 <- epcwq3[ which(epcwq3$param=="Chla"), c('date','value') ]
...
grp <- list( c(1,2), c(3,4), c(5,6), c(7,14), c(8,9) )
...
ssa_chl_otb <- list( dat = data.frame( month = chldat$month,
                                        ts = chldat$value,
...
save( ssa_chl_otb, file = "../data/ssa_chl_otb.RData" )


# Generate figure
png( "../figs/ssa_chl_otb.png",
     width = 7, height = 10, res = 500, units = "in" )
par( mfrow=c(7,1), mar=c(0,5,2,1), oma=c(2,0,0,0) )
  # data and signal
  plot( ts ~ month, dat = ssa_chl_otb$dat,
        type = 'l', las = 1,
        xlab = "", xaxt = 'n',
        ylab = "", cex.lab = 1.3,
        ylim = c(0,max(ssa_chl_otb$dat$ts)), cex.axis = 1.3,
        lwd = 2, col = rgb(0,0,0,0.8) )
  mtext( paste0( "(a) Chlorophyll-a at OTB  (signal strength: ",
                 round(ssa_chl_otb$sigstrength,2), "%)" ),
         side = 3, line = 0,
         font = 1, cex = 1.1, adj = 0 )
  date.seq <- seq.Date( min(ssa_chl_otb$dat$month),
                        max(ssa_chl_otb$dat$month)+365, 'year' )
  axis( 1, at = date.seq,
        labels = rep("",length(date.seq)), cex.axis = 1.3  )
  lines( signal ~ month, dat = ssa_chl_otb$dat,
  ...
  for( i in 1:length(recon) ){
    if( i < length(grp) ){
      # Plot signal components
      plot( recon[[i]] ~ ssa_chl_otb$dat$month, type = 'l',
