...
# OTB chl xmap TP load
ccm.otb.chl.TP <- delay.CCM( LIBRARY='chl_otb', TARGET='TP_otb',
                             EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb"),]) )
ccm.otb.chl.TP.rho <- plot.CCM( ccm.otb.chl.TP )
ccm.otb.chl.TP.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = 0 )
ccm.otb.chl.TP.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = -1 ) 
ccm.otb.chl.TP.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = -2 ) 
ccm.otb.chl.TP.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = -3 ) 
ccm.otb.chl.TP.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = -4 ) 
ccm.otb.chl.TP.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = -5 ) 
ccm.otb.chl.TP.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = -6 ) 
ccm.otb.chl.TP.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.otb.chl.TP, CCM.RHO=ccm.otb.chl.TP.rho, DELAY = -7 ) 

# NW chl xmap TP load
ccm.nw.chl.TP <- delay.CCM( LIBRARY='chl_otb_nw', TARGET='TP_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_nw"),]) )
ccm.nw.chl.TP.rho <- plot.CCM( ccm.nw.chl.TP )
ccm.nw.chl.TP.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = 0 )
ccm.nw.chl.TP.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = -1 ) 
ccm.nw.chl.TP.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = -2 ) 
ccm.nw.chl.TP.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = -3 ) 
ccm.nw.chl.TP.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = -4 ) 
ccm.nw.chl.TP.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = -5 ) 
ccm.nw.chl.TP.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = -6 ) 
ccm.nw.chl.TP.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.nw.chl.TP, CCM.RHO=ccm.nw.chl.TP.rho, DELAY = -7 ) 

# NE chl xmap TP load
ccm.ne.chl.TP <- delay.CCM( LIBRARY='chl_otb_ne', TARGET='TP_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_ne"),]) )
ccm.ne.chl.TP.rho <- plot.CCM( ccm.ne.chl.TP )
ccm.ne.chl.TP.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = 0 )
ccm.ne.chl.TP.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = -1 ) 
ccm.ne.chl.TP.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = -2 ) 
ccm.ne.chl.TP.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = -3 ) 
ccm.ne.chl.TP.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = -4 ) 
ccm.ne.chl.TP.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = -5 ) 
ccm.ne.chl.TP.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = -6 ) 
ccm.ne.chl.TP.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.ne.chl.TP, CCM.RHO=ccm.ne.chl.TP.rho, DELAY = -7 )

# CW chl xmap TP load
ccm.cw.chl.TP <- delay.CCM( LIBRARY='chl_otb_cw', TARGET='TP_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_cw"),]) )
ccm.cw.chl.TP.rho <- plot.CCM( ccm.cw.chl.TP )
ccm.cw.chl.TP.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = 0 )
ccm.cw.chl.TP.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = -1 ) 
ccm.cw.chl.TP.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = -2 ) 
ccm.cw.chl.TP.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = -3 ) 
ccm.cw.chl.TP.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = -4 ) 
ccm.cw.chl.TP.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = -5 ) 
ccm.cw.chl.TP.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = -6 ) 
ccm.cw.chl.TP.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.cw.chl.TP, CCM.RHO=ccm.cw.chl.TP.rho, DELAY = -7 ) 

# CE chl xmap TP load
ccm.ce.chl.TP <- delay.CCM( LIBRARY='chl_otb_ce', TARGET='TP_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_ce"),]) )
ccm.ce.chl.TP.rho <- plot.CCM( ccm.ce.chl.TP )
ccm.ce.chl.TP.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = 0 )
ccm.ce.chl.TP.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = -1 ) 
ccm.ce.chl.TP.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = -2 ) 
ccm.ce.chl.TP.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = -3 ) 
ccm.ce.chl.TP.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = -4 ) 
ccm.ce.chl.TP.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = -5 ) 
ccm.ce.chl.TP.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = -6 ) 
ccm.ce.chl.TP.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.ce.chl.TP, CCM.RHO=ccm.ce.chl.TP.rho, DELAY = -7 ) 

# SW chl xmap TP load
ccm.sw.chl.TP <- delay.CCM( LIBRARY='chl_otb_sw', TARGET='TP_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_sw"),]) )
ccm.sw.chl.TP.rho <- plot.CCM( ccm.sw.chl.TP )
ccm.sw.chl.TP.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = 0 )
ccm.sw.chl.TP.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = -1 ) 
ccm.sw.chl.TP.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = -2 ) 
ccm.sw.chl.TP.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = -3 ) 
ccm.sw.chl.TP.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = -4 ) 
ccm.sw.chl.TP.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = -5 ) 
ccm.sw.chl.TP.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = -6 ) 
ccm.sw.chl.TP.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.sw.chl.TP, CCM.RHO=ccm.sw.chl.TP.rho, DELAY = -7 ) 

# SE chl xmap TP load
ccm.se.chl.TP <- delay.CCM( LIBRARY='chl_otb_se', TARGET='TP_otb',
                            EMB = as.list(embed_tbl[which(embed_tbl$var=="chl_otb_se"),]) )
ccm.se.chl.TP.rho <- plot.CCM( ccm.se.chl.TP )
ccm.se.chl.TP.pval.d0  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = 0 )
ccm.se.chl.TP.pval.d1  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = -1 ) 
ccm.se.chl.TP.pval.d2  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = -2 ) 
ccm.se.chl.TP.pval.d3  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = -3 ) 
ccm.se.chl.TP.pval.d4  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = -4 ) 
ccm.se.chl.TP.pval.d5  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = -5 ) 
ccm.se.chl.TP.pval.d6  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = -6 ) 
ccm.se.chl.TP.pval.d7  <- pval.CCM( CCM.OUTPUT=ccm.se.chl.TP, CCM.RHO=ccm.se.chl.TP.rho, DELAY = -7 ) 
...
# Export outputs
save.image( file = "../data/ccm_chl00_TP.RData" )
write.csv( df.pval, file = "../data/ccm_chl00_TP_pvals.csv", row.names = FALSE )


# CCM figures --------------------------------------------------------------
png( "../figs/ccm_chl00_TP.png", width = 6, height = 10, units = 'in', res = 600 )
par(mfrow=c(4,2), mar=c(4,4.5,2,1))

plot.CCM.results( CCM.OUTPUT=ccm.otb.chl.TP,
                  CCM.RHO=ccm.otb.chl.TP.rho,
                  MAIN = "OTB segment", YLAB = TRUE,
                  PVAL=c(ccm.otb.chl.TP.pval.d0$pval, 
                         ccm.otb.chl.TP.pval.d1$pval, 
                         ccm.otb.chl.TP.pval.d2$pval, 
                         ccm.otb.chl.TP.pval.d3$pval, 
                         ccm.otb.chl.TP.pval.d4$pval, 
                         ccm.otb.chl.TP.pval.d5$pval, 
                         ccm.otb.chl.TP.pval.d6$pval, 
                         ccm.otb.chl.TP.pval.d7$pval ) )
plot(c(0,1),c(0,1),bty='n',xaxt='n',yaxt='n',xlab='',ylab='',col=rgb(0,0,0,0))
text( x = 0, y = 0.95, pos = 4, cex = 1.3,
      labels = "Chlorophyll-a xmap TP load" )
text( x = 0, y = 0.85, pos = 4, cex = 1.3,
      labels = paste0("(2000\U2013","2021)") )
...
plot.CCM.results( CCM.OUTPUT=ccm.nw.chl.TP,
                  CCM.RHO=ccm.nw.chl.TP.rho,
                  MAIN = "NW sub-segment", YLAB = TRUE,
                  PVAL=c(ccm.nw.chl.TP.pval.d0$pval, 
                         ccm.nw.chl.TP.pval.d1$pval, 
                         ccm.nw.chl.TP.pval.d2$pval, 
                         ccm.nw.chl.TP.pval.d3$pval, 
                         ccm.nw.chl.TP.pval.d4$pval, 
                         ccm.nw.chl.TP.pval.d5$pval, 
                         ccm.nw.chl.TP.pval.d6$pval, 
                         ccm.nw.chl.TP.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.ne.chl.TP,
                  CCM.RHO=ccm.ne.chl.TP.rho,
                  MAIN = "NE sub-segment",
                  PVAL=c(ccm.ne.chl.TP.pval.d0$pval, 
                         ccm.ne.chl.TP.pval.d1$pval, 
                         ccm.ne.chl.TP.pval.d2$pval, 
                         ccm.ne.chl.TP.pval.d3$pval, 
                         ccm.ne.chl.TP.pval.d4$pval, 
                         ccm.ne.chl.TP.pval.d5$pval, 
                         ccm.ne.chl.TP.pval.d6$pval, 
                         ccm.ne.chl.TP.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.cw.chl.TP,
                  CCM.RHO=ccm.cw.chl.TP.rho,
                  MAIN = "CW sub-segment", YLAB = TRUE,
                  PVAL=c(ccm.cw.chl.TP.pval.d0$pval, 
                         ccm.cw.chl.TP.pval.d1$pval, 
                         ccm.cw.chl.TP.pval.d2$pval, 
                         ccm.cw.chl.TP.pval.d3$pval, 
                         ccm.cw.chl.TP.pval.d4$pval, 
                         ccm.cw.chl.TP.pval.d5$pval, 
                         ccm.cw.chl.TP.pval.d6$pval, 
                         ccm.cw.chl.TP.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.ce.chl.TP,
                  CCM.RHO=ccm.ce.chl.TP.rho,
                  MAIN = "CE sub-segment",
                  PVAL=c(ccm.ce.chl.TP.pval.d0$pval, 
                         ccm.ce.chl.TP.pval.d1$pval, 
                         ccm.ce.chl.TP.pval.d2$pval, 
                         ccm.ce.chl.TP.pval.d3$pval, 
                         ccm.ce.chl.TP.pval.d4$pval, 
                         ccm.ce.chl.TP.pval.d5$pval, 
                         ccm.ce.chl.TP.pval.d6$pval, 
                         ccm.ce.chl.TP.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.sw.chl.TP,
                  CCM.RHO=ccm.sw.chl.TP.rho,
                  MAIN = "SW sub-segment", YLAB = TRUE, XLAB = TRUE,
                  PVAL=c(ccm.sw.chl.TP.pval.d0$pval, 
                         ccm.sw.chl.TP.pval.d1$pval, 
                         ccm.sw.chl.TP.pval.d2$pval, 
                         ccm.sw.chl.TP.pval.d3$pval, 
                         ccm.sw.chl.TP.pval.d4$pval, 
                         ccm.sw.chl.TP.pval.d5$pval, 
                         ccm.sw.chl.TP.pval.d6$pval, 
                         ccm.sw.chl.TP.pval.d7$pval ) )
plot.CCM.results( CCM.OUTPUT=ccm.se.chl.TP,
                  CCM.RHO=ccm.se.chl.TP.rho,
                  MAIN = "SE sub-segment", XLAB = TRUE,
                  PVAL=c(ccm.se.chl.TP.pval.d0$pval, 
                         ccm.se.chl.TP.pval.d1$pval, 
                         ccm.se.chl.TP.pval.d2$pval, 
                         ccm.se.chl.TP.pval.d3$pval, 
                         ccm.se.chl.TP.pval.d4$pval, 
                         ccm.se.chl.TP.pval.d5$pval, 
                         ccm.se.chl.TP.pval.d6$pval, 
                         ccm.se.chl.TP.pval.d7$pval ) )
