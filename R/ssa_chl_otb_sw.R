...
chldat1 <- epcwq3[ which(epcwq3$param=="Chla"  & epcwq3$subseg=="SW"),
                   c('date','value') ]
...
ssa_chl_otb_sw <- list( dat = data.frame( month = chldat$month,
                                        ts = chldat$value,
...
save( ssa_chl_otb_sw, file = "../data/ssa_chl_otb_sw.RData" )
