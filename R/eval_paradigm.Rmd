...
The code below loads the EPC water quality data from file (`epcwq.RData`), pivots the dataframe from wide to long format, performs QC checks, aggregates the data to a monthly timeframe (monthly means), and creates a new `epcwq2` dataframe for analysis.
...
The code below loads the Tampa Bay Estuary Program's monthly external loading estimates for Tampa Bay (`../data-raw/TB_loads_monthly.csv`), sums over TN load sources to OTB (e.g., AD, NPS, DPS, etc.) to compute total monthly TN loads, standardizes column names, and creates a new `loads` dataframe for analysis.
