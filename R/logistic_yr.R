...
# define logistic modeling function for P( y < ytarget ) ~ x1 + x2 on an annual time frame.
#   Arguments:
...
#     WY_start          the first month of the water year (integer)
#     plot              logical, displays plot if TRUE
logim_yr <- function( wqdat, loaddat,
                      y, x1, x2,
                      ytarget, xtarget,
  ...
  WY <- function( D, str = WY_start ){  # func determines water year
      if( month(D) < str ){
        return( year(D) )
      } else {
        return( year(D) + 1 )
  ...
  wqsub$year <- apply( data.frame(date=wqsub$date), 1, WY )  # assign water year
  wqsub <- wqsub |> 
  ...
  tomod <- inner_join( wqsub, loadsub, by = 'year' ) |>
...
# run models
mod.turb <- logim_yr( wqdat = epcwq3, loaddat = loads,
...
mod.sal  <- logim_yr( wqdat = epcwq3, loaddat = loads,
...
mod.temp <- logim_yr( wqdat = epcwq3, loaddat = loads,
